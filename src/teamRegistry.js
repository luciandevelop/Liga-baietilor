...
// EURO) = un nou `case` aici, nimic altundeva.
//
// IMPORTANT — Sferturi/Semifinale/Finală la Champions League NU pot
// folosi lista completă de 29: echipele reale depind de tragerea la
// sorți, necunoscută dinainte. Pentru acele 3 faze, funcția întoarce
// null — semnal explicit către Admin să introducă manual echipele reale
// calificate (mecanismul vechi, păstrat STRICT pentru acest caz).
// Winner și TOP 8 folosesc lista completă, pentru că se joacă ÎNAINTE
// de tragerea la sorți a fazei eliminatorii.
//
// "ALTA" — cerut explicit de Lu peste tot, nu doar la EL/UECL: oricare
// fază de tip "cine câștigă X" (single-pick, listă cunoscută dinainte)
// primește opțiunea ALTA. Fazele eliminatorii (Sferturi/Semifinale/
// Finală) NU primesc ALTA — la acelea, adminul intră DOAR echipele reale
// calificate; "altă echipă" nu are sens quando lista e deja restrânsă
// la cine chiar s-a calificat.
export function resolveTeamOptions(competitionId, phaseId) {
  ...
  switch (competitionId) {
    ...
    case "la-liga":
      return [...LA_LIGA_TEAMS, ALTA_OPTION];
    case "serie-a":
      return [...SERIE_A_TEAMS, ALTA_OPTION];
    case "superliga":
      return [...SUPERLIGA_TEAMS, ALTA_OPTION];
    default:
      return [];
  }
}
